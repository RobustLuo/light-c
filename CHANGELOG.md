...
- **NSIS 安装向导品牌图修复**：`gen-nsis-assets` 改为优先使用与 exe 一致的 `icon-source.png` 生成 `wizard.bmp` / `header.bmp`，安装侧边栏不再显示旧版绿色「C:」占位图
- **CI Release 路径转义修复**：写入 `GITHUB_ENV` 的 Windows 路径改用正斜杠，避免 `\t`、`\r` 被误解析导致 Configure 步骤失败
- **CI Release 产物路径解析**：新增 `ci-resolve-release-paths.ps1`，构建后立即备份 runtime exe，修复签名/便携版步骤找不到 `LuoScope.exe`
- **官方下载渠道调整**：设置 → 安全与校验改为「GitHub Releases + 微信公众号」双渠道展示；公众号内置扫码关注二维码
- **垃圾软件清理（新模块）**：通过注册表 Uninstall 项 + 特征库识别 360、鲁大师、2345、腾讯电脑管家、金山、驱动人生/精灵等常见捆绑软件；扫描结果默认不勾选，用户确认后依次调用官方卸载程序（优先静默命令）；卸载后建议在「卸载残留」「注册表冗余」模块复查
- **磁盘变化分析引导区美化**：扫描前合并盘符快照、功能说明与提权提示；复用顶栏磁盘卡与主题令牌，去掉嵌套高对比色块与重复扫描按钮
- **项目目录与发版资产命名统一为 LuoScope**：发版 Skill 目录改为 `skills/luoscope-release`；本地 Tauri 签名公钥/私钥默认文件名改为 `.tauri/luoscope.key`（仍兼容旧 `lightc.key`）；建议将仓库根目录文件夹命名为 `luo-scope` 与 npm 包名一致
- **应用图标黑边修复**：图标生成脚本对源图套用圆角 Alpha 遮罩，四角黑底改为透明，修复 Windows 桌面/任务栏显示黑边
- **开发缓存迁移到 D 盘**：Cargo 编译产物、Cargo 依赖、npm 缓存与链接临时文件默认写入 `D:\DevCache\LuoScope\`，避免 C 盘空间不足导致编译失败
- **D 盘缓存一键迁移脚本**：新增 `scripts/setup-d-drive-cache.ps1`，迁移 `~\.cargo` / `npm-cache`，并为 Cursor 沙盒目录 `cursor-sandbox-cache` 建立 C→D 目录联接 + 用户环境变量，防止 Agent 编译再次占满 C 盘
- **发版签名密钥脚本**：新增 `scripts/generate-signing-key.ps1`（`npm run setup:signing-key`），用于首次生成 `.tauri/luoscope.key` 并同步公钥到 `tauri.conf.json`
- **品牌更名为 LuoScope**：产品名、可执行文件（`LuoScope.exe`）、安装包与便携包资产统一更名；`%LOCALAPPDATA%/LuoScope` 为新数据目录，自动迁移旧版 `%LOCALAPPDATA%/LightC` 配置与运行数据；便携 marker / manifest、WebView2 与 localStorage 均兼容旧版 LightC
- **深色模式图标视觉优化**：侧栏、设置页与模块/空状态图标移除外发光，改用柔和蓝灰描边胶囊风格，降低霓虹感
- **关于页精简**：移除「更新日志」「源码地址」「源码许可证」外链入口
- **空状态占位美化**：去掉虚线框与廉价渐变，改为毛玻璃 + 背景光斑；移除图标外圈与弹跳动效；取消模块卡片 hover 上浮
- **欢迎弹窗美化**：单页自然排版，纵向亮点列表替代三列小卡片；标语改为中性胶囊、全宽主按钮
- **确认清理弹窗可读性优化**：免责声明改为高对比正文 + 独立标签；危险确认按钮去渐变/外发光
- **操作进度遮罩可读性优化**：清理/删除过程提示改为胶囊加载圈 + 全宽流光条；底部提示改用高对比胶囊，去掉外发光
- **大屏布局适配**：页面模式内容区纵向撑满视口，模块卡片/空状态/扫描占位随高度自适应；1280px+ 放宽内容最大宽度并略增侧栏宽度，顶栏与主内容对齐
- **分栏布局模式（新）**：设置 → 通用新增第三种布局「分栏模式」——左侧加宽侧栏展示「扫描总览」与各模块状态胶囊（项数/大小/扫描中）；与页面模式同位置但信息更丰富，适合逐项清理；设置页改用带线框预览的三选一卡片
- **模块扫描态统一美化**：新增 `ModuleScanPanel` / `ModuleScanStatusBar`，各清理/分析模块扫描中改用毛玻璃 + 环形指示 + 统计 chips + 底部流光；卡片头部「扫描中」改为脉冲点徽章，扫描按钮仅保留图标避免重复文案
- **大目录分析结果布局重构**：统计摘要改为三列 chips；目录列表改为扁平树形行 + 细占比条，去掉嵌套色块与立体文件夹图标；操作按钮常显低对比、悬停高亮
- **大目录分析视觉增强**：摘要区光斑 + 图标 chips；Top3 金/银/铜序号与行底色；列表表头、路径等宽字体、占比百分比、大小胶囊、子目录虚线引导与悬停左侧色条
- **一键扫描范围可配置**：设置 → 功能设置新增模块多选；新安装默认仅启用垃圾清理、大文件、社交专清、大目录；旧版用户未写入该配置时仍全部参与，避免升级后行为突变
- **垃圾清理 / 右键菜单扫描占位统一**：扫描中改用 `ModuleScanPanel`，与其他分析模块视觉一致
- **一键扫描监听统一**：各模块改用 `useOneClickScanListener`，移除分散的 trigger/ref 逻辑
- **高端过渡动效增强**：侧栏/设置导航新增滑动胶囊指示器；页面模式切换改为整体 blur 淡入 + 卡片错峰上浮（移除顶部扫光蓝线）；手风琴展开、空状态/扫描占位、大目录结果列表支持错峰淡入；设置 Tab 切换增加轻量缩放过渡；全程尊重 `prefers-reduced-motion`
- **盘符/筛选下拉美化**：下拉菜单改为实底毛玻璃 + 强阴影，避免模块正文穿透；盘符选择器随卷标自适应宽度，选中项改用勾选标记
- **Toast 通知动效**：从右侧完整滑入/滑出，入退场路径对称，时长与 JS 生命周期对齐
- **主窗口默认尺寸调整**：默认 1200×800（主流桌面工具常见开屏尺寸），最小 1024×680，侧栏与模块布局更易读
- **页面模式空状态优化**：扫描前空状态垂直居中，内置「开始扫描」主按钮；idle 时隐藏重复页面提示；侧栏导航略增内边距；修复页面模式底部留白/白边（移除 body pb、叠加 min-height 与多余 padding）
- **顶栏存储空间重构**：展示全部固定盘（C/D 等）与 U 盘，横向滚动卡片 + 使用率色阶；`get_local_drives` 支持可移动存储；扫描模块盘符选择仍仅固定磁盘
- **顶栏存储空间布局优化**：1–3 个磁盘时卡片均分宽度消除右侧空白；标题行增加全盘可用/总容量汇总；系统盘使用率 >75% 或可用 <20GB 时右侧显示「空间偏紧」洞察条，可一键跳转垃圾清理 / 大文件

- **标题栏布局优化**：品牌 / 拖拽区 / 工具胶囊 / 窗口控件三段式布局；主题切换与设置合并为统一工具栏，单行品牌信息更紧凑
- **关于页与标题栏品牌区美化**：统一使用新图标 `AppBrandLogo`；关于页改为毛玻璃主视觉卡片 + 系统信息网格布局
- **应用图标更新**：替换为全新 C 盘清理主题图标（C: + 磁盘 + 清理光效），并重新生成 Tauri 多尺寸 PNG / ICO 与 `public/logo.png`
- **启动屏视觉升级**：移除旧版扫描线与英文校验文案，改为白底毛玻璃卡片 + 主题色光晕与进度条，自动跟随浅色/深色主题
- **启动屏重新设计**：复用 AppBrandLogo；标语改为胶囊标签；三阶段圆点 + 同步进度条；状态/页脚对比度提升；去掉脉冲光晕与进度外发光
- **启动屏再次美化**：全幅毛玻璃窗口 + 网格光晕背景；去掉重复胶囊/badge 与圆点指示器；品牌区居中放大 Logo；底部 Dock 整合状态文案、步进序号与细线进度条；标语改为轻量字距分隔排版
- **管理员权限一键提权**：设置 → 安全新增「启动时自动请求管理员权限」开关；非管理员时可一键 UAC 提权重启（正式版）；开发模式需以管理员终端运行 `npm run tauri dev`；提权后 WebView2 使用独立数据目录避免 0x80070057
- **顶栏健康评分优化**：独立毛玻璃卡片 + 分项进度条，标签与数值分层更清晰
- **操作进度遮罩统一**：清理/删除过程改用 `OperationProgressOverlay` 简约毛玻璃提示，环形指示 + 细线流光，统一丝滑入退场动画
- **设置页 UI 升级**：毛玻璃侧栏导航 + 统一设置卡片/分段控件；打开/关闭与切换 Tab 增加丝滑过渡动画
- **全局动效体系**：统一 motion 令牌与 `useOverlayAnimation`；弹窗/Toast/页面切换/手风琴/卡片悬停全面丝滑过渡
- **模块卡片头部重构**：双行布局（信息 + 工具栏），统计胶囊与操作按钮不再挤在一行

### 左侧固定导航栏

- **固定侧栏**：原左侧悬浮图标改为 220px 宽毛玻璃导航栏，图标 + 文字始终可见，空间搭配更稳定。
- **双模式适配**：页面模式点击切换模块；卡片模式点击滚动到对应锚点并自动高亮当前模块。

### 白底毛玻璃主题

- **浅色默认白底**：应用默认进入浅色模式，面板采用半透明白色与强 blur 毛玻璃效果。
- **深浅色切换**：标题栏新增主题切换按钮；设置 → 通用中仍可切换浅色 / 深色 / 跟随系统。
- **毛玻璃分层**：标题栏、顶栏、模块卡片、设置/欢迎/更新弹窗统一使用 `glass-panel` / `glass-panel-strong` 材质。

### Aurora UI 视觉升级

- **全新 Aurora 设计系统**：采用天空蓝主色、玻璃材质标题栏/导航、分层阴影与网格光晕背景，替换原微信绿扁平风格。
- **动效规范对齐**：模态框/手风琴/页面切换使用强 ease-out 与 drawer 曲线；按钮增加按压缩放反馈，并支持 `prefers-reduced-motion`。
- **核心壳层重构**：标题栏、仪表盘顶栏、模块卡片、侧边导航、欢迎弹窗与启动页同步适配新令牌。

### Fork 维护者信息

- **作者信息更新**：界面、配置、更新校验与发布流程中的作者署名统一调整为 RobustLuo，GitHub 仓库指向 `RobustLuo/light-c`。
- **应用标识更新**：Tauri 包名调整为 `com.robustluo.LightC`，并保留对旧版 `com.chunyu.LightC` WebView2 数据的迁移兼容。
- **渠道信息收敛**：移除原项目 B 站、抖音、网盘与赞赏入口，反馈与下载渠道统一回落到 RobustLuo 的 GitHub。
