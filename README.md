...
- **删除前备份**：删除前先导出驱动包到当前 LightC 数据目录的独立 `driver_backups` 文件夹，备份失败会阻止删除
...
- **稳定布局**：主窗口初始宽度与最小宽度保持一致，磁盘变化分析概览指标固定四列展示，避免关键指标在窄窗口下换行
...
LightC 采用 [LightC Source Available License v1.0](LICENSE)。
