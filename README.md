<p align="center">
  <img src="src-tauri/icons/icon.png" width="128" height="128" alt="LuoScope Logo">
</p>

<h1 align="center">LuoScope</h1>

<p align="center">
  <strong>Windows 智能磁盘空间管理</strong>
</p>
...
<p align="center">
  <img src="public/assets/show1.png" alt="LuoScope Screenshot" width="900">
  <img src="public/assets/show2.png" alt="LuoScope Screenshot" width="900">
  <img src="public/assets/show3.png" alt="LuoScope Screenshot" width="900">
  <img src="public/assets/show4.png" alt="LuoScope Screenshot" width="900">
  <img src="public/assets/show5.png" alt="LuoScope Screenshot" width="900">
  <img src="public/assets/show6.png" alt="LuoScope Screenshot" width="900">
  <img src="public/assets/show7.png" alt="LuoScope Screenshot" width="900">
</p>

## ✨ 功能特性

LuoScope **全部功能免费**，无 Pro 付费档位。若软件对你有帮助，可在 **设置 → 意见反馈 → 支持作者** 自愿赞赏，或关注微信公众号获取更新通知。
...
- **扫描范围可配置**：可在「设置 → 功能设置 → 一键扫描」勾选参与模块；新安装默认仅垃圾清理、大文件、社交专清、大目录，旧版用户未写入该配置时仍全部参与
- **扫描停止控制**：扫描过程中可随时点击停止按钮终止所有扫描任务
- **回收站安全清空**：通过 Windows Shell API 清空回收站，支持所有驱动器的回收站文件一键清理
- **回收站显示一致**：只扫描当前用户可见且 `$I` 元数据与 `$R` 数据完整配对的回收站条目，过滤其他用户、孤儿文件、`desktop.ini` 和内部元数据
- **回收站条目可读**：按 Windows `$I` 元数据恢复原始文件名和原始路径，不显示 `$I/$R` 内部文件名；打开操作进入系统回收站视图
- **应用数据保护**：不再泛扫 `Packages\*\LocalCache`，并跳过 WebView2/Chromium 应用 Profile 的持久化数据库，避免误删 Claude 等桌面应用的会话索引和设置
- **低功耗清理遮罩**：清理执行中移除高成本背景模糊并保留轻量旋转提示，降低高刷新率屏幕上 WebView2 的持续合成压力
- **虚拟列表优化**：大量文件列表也能流畅滚动
...
- **多盘扫描**：支持在模块标题旁切换目标分区，MFT 优先、WalkDir 降级，并用最小堆维护 Top N 最大文件
- **可调扫描量**：可在「设置 → 功能设置 → 大文件清理」中自定义返回数量（10-500，默认 50）
...
### 🧩 旧驱动清理（需管理员权限）
- **官方工具检测**：通过 Windows `pnputil` 的结构化 XML 输出读取第三方驱动包，不直接扫描或删除 `DriverStore`、`.sys` 文件
- **系统版本兼容**：旧版 Windows 不支持 `pnputil` XML 参数时自动回退到文本输出，不会因格式参数缺失而无法检测驱动包
- **Windows 10 文本兼容**：旧驱动清理兼容中文系统字段、字段冒号空格和无 BOM UTF-16 输出，适配 Windows 10 22H2 等环境
- **保守删除边界**：全量检测第三方驱动包，并单独统计可处理候选；正在使用的驱动包绝对不可选，未关联设备但无法证明已被替代的条目只允许用户确认后处理，驱动类别提供中文对照
- **失败可定位**：删除失败会在结果提示中展示 `pnputil` 返回原因；重复尝试会复用已有驱动备份，不重复导出同一驱动包
- **辅助确认**：每个驱动条目支持搜索驱动信息和打开 Driver Store 所在目录，搜索关键词会自动去除日期等噪声
- **高置信判定**：额外读取 `pnputil` 的设备驱动匹配和排名；只有未被设备使用且在匹配列表中明确 `Outranked` 的包才标记为“高置信旧驱动”，其余仍需人工确认
- **快捷操作**：顶部操作栏提供独立的“高置信”按钮，可切换选中/取消全部高置信旧驱动；未滚动时撑满列表宽度，吸顶后自动收缩为紧凑布局
- **删除前备份**：删除前先导出驱动包到当前 LuoScope 数据目录的独立 `driver_backups` 文件夹，备份失败会阻止删除
- **二次复核**：后端删除前重新检测设备绑定，删除后再次扫描确认驱动包是否移除，不使用 `/force`、不自动重启
- **手动恢复**：软件不保存驱动删除历史，也不提供自动恢复状态；备份可在设置中的本地数据清理项中手动清理，但默认不会勾选
- **一键恢复备份**：可在模块顶部读取当前数据目录 `driver_backups` 中的全部备份，并交由 Windows `pnputil` 递归安装；恢复前需要管理员权限和二次确认
- **后台检测**：`pnputil` 检测和磁盘信息 PowerShell 查询使用脱离控制台与无窗口创建标志，不会在发布包的管理员环境中闪现命令行窗口
- **兼容性修复**：磁盘信息查询使用 Windows PowerShell 的 UTF-16LE 编码命令和标准输出写入，降低不同管理员/发布环境下返回空结果的风险

### 🛡️ 垃圾软件清理（建议管理员权限）
- **注册表特征匹配**：读取 `Uninstall` 注册表项，通过内置特征库识别 360、鲁大师、2345、腾讯电脑管家、金山、驱动人生/精灵等常见捆绑/推广软件
- **可信发布者过滤**：自动跳过 Microsoft、NVIDIA、Intel 等正规厂商条目，降低误报
- **用户确认卸载**：扫描结果默认不勾选；确认后依次调用官方 `UninstallString` / `QuietUninstallString`，优先静默命令
- **卸载后复查**：建议在「卸载残留」「注册表冗余」模块再次检查残留文件夹与注册表项

### 🔬 大目录分析
- **双模式扫描**：默认 AppData 智能分析（快速定位用户数据热点），深度扫描模式可选择 C/D/E 等目标磁盘并覆盖该盘一级目录
...
- **系统目录过滤开关**：深度扫描时可选择是否扫描系统保护目录（Windows、Program Files 等），关闭后系统目录会参与 TopN 排名，便于直接看到所选磁盘下的 Windows、WinSxS、System32 这类占用大户
...
### 📂 磁盘变化分析
- **扫描前引导**：未扫描时展示统一毛玻璃引导卡片，集成盘符快照、MFT/快照对比说明与紧凑提权提示，避免空白灰条与重复按钮堆叠
- **分区选择**：启动时读取本机固定磁盘分区，支持在模块标题旁选择 C/D/E 等 NTFS 分区进行磁盘变化分析；非 NTFS 分区会展示但禁止 MFT 扫描，避免用户误以为扫描失败
- **MFT 快速扫描**：原 ProgramData 分析入口升级为多盘磁盘变化分析，管理员运行时通过 NTFS MFT 枚举重建目录树，并顺序扫描 `$MFT` 建立文件大小表，避免传统全盘递归扫描和大量逐文件 metadata IO
- **可取消扫描**：模块内停止按钮与顶部全局“停止扫描”都会通知后端中断 MFT 扫描，取消后不会展示半截结果
- **快照对比**：每次扫描都会按所选磁盘保存目录聚合快照，并与同一磁盘的上一次快照对比，计算净新增/净减少空间；C 盘继续沿用 `disk_growth_*` 历史文件名，D/E 等其他盘使用 `d_disk_growth_*`、`e_disk_growth_*` 前缀，统一存放在 `disk_growth_snapshots/` 下；文件级明细采用同名分片目录存储，避免多 TB 磁盘下生成或读取超大主 JSON；每个磁盘快照最多保留 3 组，超过后自动清理旧主快照和同名分片目录
- **变化目录定位**：结果列表优先展示本次与上次之间发生变化的目录、真实变化时间、当前大小、变化量、变化级别和原因提示，支持一键打开目录或按设置中的搜索引擎搜索路径用途继续排查；变化项按变化量排序，并自动折叠变化量完全一致的冗余父级目录，默认最多展示 300 个目录，可在设置中调整为 50-1000
...
- **稳定布局**：主窗口默认 1200×800、最小 1024×680，兼顾主流开屏观感与侧栏模块可读性；磁盘变化分析概览指标固定四列展示
...
- **超大磁盘适配**：文件级明细按分片懒加载，几个 TB 的磁盘也不会因为打开明细而一次性读取超大快照；实际扫描耗时主要取决于文件数量、`$MFT` 体积、硬盘类型和安全软件实时扫描，应用启动后的首次 MFT 扫描可能因系统缓存预热而更慢
- **模块化实现**：全盘扫描、快照和增长分析代码已迁移到 `src-tauri/src/disk_growth/`，命令入口统一为 `scan_disk_growth`

### 💽 磁盘信息
- **基础硬件信息**：批量读取本机物理磁盘型号、容量、SSD/HDD、NVMe/SATA/USB 等总线、固件版本和脱敏序列号，并关联盘符、卷标、文件系统与空间使用情况
- **健康状态**：展示 Windows Storage/设备报告的健康、警告、异常或未知状态；不伪造 SMART 寿命百分比，“未知”不代表健康或故障
- **性能与边界**：一次查询全部磁盘，Rust 使用阻塞线程执行并设置超时；功能只读，不执行修复、TRIM、格式化、坏道扫描或其他写操作
- **使用方式**：打开「设置 - 磁盘信息」即可读取；需要最新状态时手动点击“重新读取”
- **运行期缓存**：不会因为切换设置菜单自动重复查询；用户手动读取后，本次打开软件期间复用当前快照，重新读取才替换旧数据，不写入本地存储
...
- **结果展示**：提供总占用、最大模型、超过 20GB 的模型数量、概览图表、模型列表筛选、关键词搜索、一键清空搜索、打开目录、一键搜索模型和单文件删除能力；删除前必须经过二次确认，后端会再次校验模型格式并复用增强删除引擎；平台标签使用主色实心样式，类型标签统一展示文件扩展名，“未归类”用暖色提醒，长路径采用中间省略以尽量保留模型文件名；同一路径命中多个来源时按平台优先级去重，避免重复计数
...
- **配置与数据分离**：安装版配置固定保存在 `%LOCALAPPDATA%/LuoScope/config/config.json`，默认运行数据保存在 `%LOCALAPPDATA%/LuoScope/data/`；便携版默认将 `config/` 和 `data/` 保存在 `LuoScope.exe` 同目录，移动整个便携目录后数据仍然跟随移动
...
- **数据目录迁移保护**：更改数据目录时必须选择独立空文件夹，禁止选择当前数据目录、父目录或子目录；迁移只复制 LuoScope 明确拥有的日志、备份、快照和安装历史缓存，避免误选磁盘根目录后把无关文件继续迁移
...
### 🎨 现代化界面
- **白底毛玻璃 UI**：默认浅色白底，面板采用半透明毛玻璃 + 强 blur；深色模式为暗色毛玻璃；标题栏可快速切换浅色 / 深色 / 跟随系统
- **自定义标题栏**：无边框窗口设计，与主题色完美融合
- **深色/浅色主题**：支持跟随系统或手动切换（设置 → 通用）
- **字体大小调节**：支持标准/适中/较大三档字号，满足不同视力需求
- **左侧固定导航栏**：220px 毛玻璃侧栏常驻显示，模块图标 + 名称一目了然；**卡片模式**锚点滚动、**页面模式**切换单模块、**分栏模式**左侧状态列表 + 右侧专注操作（设置 → 通用可切换三种布局，带线框预览）
- **大屏布局**：页面/分栏模式下模块区域随窗口高度撑满，空状态居中分布并内置扫描入口；宽屏下内容区与顶栏同步放宽
- **顶栏存储空间**：展示全部固定盘（C/D 等）与已插入 U 盘，横向滚动卡片 + 使用率色阶；盘少时卡片均分宽度；标题汇总全盘可用/总容量；系统盘偏紧时右侧洞察条可跳转垃圾清理 / 大文件；扫描模块盘符选择仍仅固定磁盘
...
- **流畅动画**：侧栏/设置导航滑动胶囊指示器、页面模式整体 blur 淡入 + 卡片错峰上浮、手风琴内容展开、空状态/扫描占位与大目录结果错峰入场；全程支持 `prefers-reduced-motion`
- **响应式布局**：适配不同窗口尺寸
- **反馈入口**：设置页反馈统一指向 RobustLuo 的 GitHub Issues 与作者主页
...
### 🎬 启动动画
- **毛玻璃启动屏**：全幅毛玻璃 + 网格光晕；居中品牌 Logo 与标题；底部 Dock 展示加载状态与细线进度；自动跟随浅色/深色主题
- **品牌标识**：启动页底部展示「RobustLuo · 官方维护」
- **双窗口架构**：Tauri 2.0 splashscreen + main 窗口分离，启动体验更流畅

### 🔐 安全与校验
- **官方版本安全声明**：设置中独立选项卡，警示第三方打包风险（捆绑插件、主页劫持、后门程序）
- **一键完整性校验**：设置 → 安全与校验 可直接验证当前 LuoScope.exe 是否通过官方 ed25519 签名；常规安装包、WebView2 离线安装包和便携版分别使用 Release 中的 exe 专用签名资产
- **签名格式兼容**：校验逻辑会兼容历史 Release 中被双重 base64 包装的签名资产，并把签名资产异常与文件内容不一致分开提示，降低官方包误报风险
- **版权与渠道声明**：设置中标注 RobustLuo 维护的 GitHub Releases 官方发布渠道，并提示第三方转载、二次打包风险
...
│   │   └── index.ts                  # 组件统一导出
│   ├── settings/                      # 设置弹窗的一级菜单页面
│   │   └── DiskInfoSettings.tsx       # 磁盘信息设置页
│   ├── contexts/
...
│   │   └── index.ts
│   ├── stores/
│   │   └── diskInfoStore.ts           # 磁盘信息运行期缓存（Zustand）
│   ├── hooks/
...
2. **管理员权限**：MFT 全盘分析、系统瘦身、旧驱动清理等需要管理员权限。可在 **设置 → 安全** 开启「启动时自动请求管理员权限」，或在模块内点击「以管理员身份重启」通过 UAC 提权（Windows 不允许静默绕过 UAC）
3. **谨慎删除**：高风险分类（如旧Windows安装）删除后无法恢复

### 开发相关

1. **缓存目录在 D 盘**：开发/编译默认使用 `D:\DevCache\LuoScope\`（Cargo 编译产物、Cargo 依赖缓存、npm 缓存、临时文件、Cursor 沙盒缓存）；通过 `scripts/set-dev-cache-env.ps1` 注入，`npm run dev` / `npm run tauri dev` 会自动生效。**首次建议**执行 `.\scripts\setup-d-drive-cache.ps1`，将 `~\.cargo`、`npm-cache` 迁到 D 盘，并用目录联接把 `%LOCALAPPDATA%\Temp\cursor-sandbox-cache` 重定向到 D 盘，避免 Agent 沙盒再次占满 C 盘
2. **首次编译较慢**：Rust 首次编译需要下载和编译大量依赖，请耐心等待
3. **热重载**：前端支持热重载，Rust 代码修改需要重新编译
4. **调试**：开发模式下可使用 `F12` 打开开发者工具
...
3. 参考项目内 `skills/luoscope-release/SKILL.md` 执行发版前检查，确保版本号、发布说明和 README 行为描述一致
4. 构建并签名
5. 上传到 GitHub Releases：
   - `LuoScope_x.x.x_x64-setup.nsis.zip`
   - `LuoScope_x.x.x_x64-setup.nsis.zip.sig`
   - `latest.json`（构建时自动生成）
   - `LuoScope_webview2_offline_x64.exe`（内置 WebView2 离线安装器，适合 WebView2 环境异常或安装时网络受限的用户）
   - `LuoScope_portable_x64.zip`（便携包内包含 `LuoScope.portable` 旧标记和 `LuoScope.portable.json` manifest；默认数据保存在程序目录）
   - `LuoScope_installer_exe.sig`（安装版用于校验当前 `LuoScope.exe` 的官方签名，格式与 updater 的 base64 签名字符串一致）
   - `LuoScope_webview2_offline_exe.sig`（WebView2 离线安装版用于校验当前 `LuoScope.exe` 的官方签名）
   - `LuoScope_portable_exe.sig`（便携版解压后用于校验当前 `LuoScope.exe` 的官方签名，格式与 updater 的 base64 签名字符串一致）
   - `download.json`（官方渠道配置：GitHub Releases、微信公众号名称/链接等；便携版更新入口与设置 → 安全与校验会动态读取）

### 便携版更新策略

- 常规安装包不再联网自动下载 WebView2 Runtime；安装前会检测本机 WebView2，缺失时提示用户改用 `LuoScope_webview2_offline_x64.exe` 或先安装 WebView2 Runtime，避免微软 bootstrapper 在部分环境里创建额外 Edge 快捷方式。
- 安装版保留 Tauri 自动更新，继续使用 `latest.json` 和签名包完成更新；自动更新或覆盖安装前会尝试关闭残留的 `LuoScope.exe`，降低目标 exe 被占用导致安装失败的概率。
- WebView2 异常或安装时网络受限的用户可改用 `LuoScope_webview2_offline_x64.exe`。
- 完整性校验不复用更新包签名，而是读取当前版本 Release 中的 exe 专用签名资产，避免把 zip/updater 包签名误用于运行中 exe。
- 便携版由发布流程写入 `LuoScope.portable.json` manifest，并保留 `LuoScope.portable` 旧标记；运行时识别后不会自动弹出更新安装器。旧版本便携包仍可被新版识别。
- 便携版完整性校验读取 `LuoScope_portable_exe.sig`，校验对象是便携包内最终的 `LuoScope.exe`；安装版和 WebView2 离线版分别使用各自的 exe `.sig` 文件，不能混用。
- 设置 → **安全与校验 → 官方下载渠道** 展示 **GitHub Releases** 与 **微信公众号**（内置扫码关注二维码）
- 便携版「检查更新」入口会优先读取 Release 的 `download.json` 并打开作者网盘下载页（若配置了 `netDiskUrl`），用户下载新版 zip 后覆盖当前目录即可；读取失败时降级到 GitHub Releases。
...
1. 打开 **设置 → 安全**，开启「启动时自动请求管理员权限」，或在模块内点击 **「以管理员身份重启」**
2. 在 Windows UAC 弹窗中点击 **「是」** 确认提权
3. 进入 **系统瘦身** 页面，根据需要点击各项的操作按钮

> 也可继续通过右键程序图标 → **「以管理员身份运行」** 手动启动。
...
LuoScope 采用 [LuoScope Source Available License v1.0](LICENSE)。

LuoScope 公开源码的目的，是让用户可以审计代码、确认软件行为透明可信，并让开发者能够阅读、学习、Fork、修改源码和向官方仓库提交 Pull Request。
...
- 将 LuoScope 或其修改版本作为独立产品发布
- 改名、换标、二次打包或发布非官方版本
- 销售、出租、托管、再许可或以其他方式商业化 LuoScope
- 将 LuoScope 作为商业产品、付费服务、捆绑软件或专有发行的一部分再分发
- 删除、隐藏或修改版权、许可证、作者署名、官方渠道或安全校验相关信息

当前发行版本为全功能免费；作者保留在未来另行提供其他许可或商业安排的权利。
